...
## 2. 요구사항 정의서 (DevOps 도입 관련 추가)

#### 2.1 기능적 요구사항

| 요구사항 ID | 요구사항 내용 | 우선순위 | 담당자 | 상태 | 비고 |
| --- | --- | --- | --- | --- | --- |
| DV-001 | 서비스 디스커버리(Eureka) 구축 | 높음 ⬆️ | 이도현 | 완료 ✅ | 각 마이크로서비스 자동 등록/탐색 |
| DV-002 | API Gateway 도입 | 높음 ⬆️ | 이도현 | 완료 ✅ | 통합 진입점, 라우팅 및 인증 |
| DV-003 | 설정 관리(Config Server) | 높음 ⬆️ | 이도현 | 완료 ✅ | 공통설정 중앙화, 동적 리프레시 |
| DV-004 | CI/CD 파이프라인 구축 | 높음 ⬆️ | 전체 | 완료 ✅ | GitHub Actions 기반 자동화 |
| DV-005 | Docker 기반 컨테이너화 | 높음 ⬆️ | 전체 | 완료 ✅ | 서비스별 Dockerfile 작성 |
| DV-006 | 배포 자동화(서버 배포스크립트) | 높음 ⬆️ | 전체 | 완료 ✅ | docker-compose 이용 |
| DV-007 | 기본 모니터링(로그, 상태체크) | 중간 | 전체 | 예정 ❌ | health check, slack 알림 등 |
| DV-008 | 롤백/재배포 기능 | 중간 | 전체 | 예정 ❌ | 실패시 이전 이미지 복구 |
| DV-009 | 분산 트랜잭션 관리/로그 추적 | 낮음 | 전체 | 예정 ❌ | 향후 ELK, CloudWatch 등 확장 가능 |

#### 2.2 비기능적 요구사항

| 요구사항 ID | 요구사항 내용 | 우선순위 | 담당자 | 상태 | 비고 |
| --- | --- | --- | --- | --- | --- |
| NF-001 | 확장성(수평적 확장 지원) | 높음 ⬆️ | 전체 | 완료 ✅ | 컨테이너 추가로 서비스 확장 가능 |
| NF-002 | 가용성(서비스 다운타임 최소화) | 높음 ⬆️ | 전체 | 완료 ✅ | 장애시 빠른 재기동/롤백 |
| NF-003 | 응답시간(평균 1초 이내) | 높음 ⬆️ | 전체 | 진행중 ⬜️ | API Gateway 기준 |
| NF-004 | 보안(시크릿 관리, 포트 제한) | 높음 ⬆️ | 이도현 | 완료 ✅ | GitHub Secrets, 보안그룹 등 |
| NF-005 | 컨테이너 이미지 취약점 점검 | 중간 | 전체 | 예정 ❌ | DockerHub 취약점 검사 활용 예정 |
| NF-006 | 로그/모니터링/알림 | 중간 | 전체 | 예정 ❌ | S3/CloudWatch/Slack 등 연동 |

#### 2.3 운영 요구사항

| 요구사항 ID | 요구사항 내용 | 우선순위 | 담당자 | 상태 | 비고 |
| --- | --- | --- | --- | --- | --- |
| OP-001 | CI/CD 파이프라인 자동화 | 높음 ⬆️ | 전체 | 완료 ✅ | main 브랜치 푸쉬 → 자동 배포 |
| OP-002 | 무중단 배포(롤링/블루그린 구조) | 중간 | 전체 | 예정 ❌ | 단일서버 기준 rolling 적용 |
| OP-003 | 롤백 기능(자동/수동) | 높음 ⬆️ | 전체 | 예정 ❌ | 배포실패시 이전 이미지 복원 |
| OP-004 | 배포/운영 이력 관리 | 중간 | 전체 | 예정 ❌ | 배포 로그 및 히스토리 관리 |
| OP-005 | 상태/헬스체크 자동화 | 중간 | 전체 | 예정 ❌ | 배포후 Health Check, Slack 알림 |
| OP-006 | 환경별(.env, Secret) 분리 운영 | 높음 ⬆️ | 이도현 | 완료 ✅ | 운영/개발 환경 구분 |
